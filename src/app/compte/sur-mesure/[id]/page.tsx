...
export default function SurMesureDetailPage() {
  ...
  const sm = mySurMesure.find((s) => s.id === id);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "#b08a4a99" }}>#{sm.id} · Demande du {sm.date}</p>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 rounded-2xl" style={{ background: "#fef9c3" }}>
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "#a16207" }}>
              Un acompte de <strong>{sm.acompte}</strong> est requis pour lancer la production.
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-xs" style={{ color: "#b08a4a99" }}>{doneCount}/{sm.pipeline.length} étapes</span>
        </div>
        ...
        <div className="h-2 rounded-full mb-6 overflow-hidden" style={{ background: "#f0ebe0" }}>
          <div
            ...
            style={{ width: `${(doneCount / sm.pipeline.length) * 100}%`, background: "#b08a4a" }}
          />
        </div>

        <div className="space-y-0">
          {sm.pipeline.map((step, i) => {
            const isLast = i === sm.pipeline.length - 1;
            const isActive = step.done && (isLast || !sm.pipeline[i + 1]?.done);
            return (
              <div key={i} className="flex gap-4">
                <div className="flex flex-col items-center">
                  ...
                  {!isLast && (
                    <div className="w-0.5 flex-1 my-1" style={{ background: step.done && sm.pipeline[i + 1]?.done ? "#b08a4a" : "#e8d9bd", minHeight: "24px" }} />
                  )}
                </div>
                <div className="pb-5 flex-1">
                  ...
                  {!step.done && !isActive && (
                    <p className="text-xs mt-0.5" style={{ color: "#d1d5db" }}>En attente</p>
                  )}
                </div>
              </div>
            );
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold" style={{ color: "#14110d" }}>{sm.devis}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span style={{ color: "#b08a4a99" }}>Acompte (50%)</span>
            <span className="font-semibold" style={{ color: "#14110d" }}>{sm.acompte}</span>
          </div>
